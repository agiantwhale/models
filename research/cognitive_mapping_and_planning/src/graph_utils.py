...
def _get_next_nodes(n, sc, size):
    (p, q) = n

    return [(x, y)
            for x, y in [(p + sc, q), (p, q + sc), (p - sc, q), (p, q - sc)]
            if 0 <= x <= size[0] and 0 <= y <= size[0]]


def generate_graph(valid_fn_vec=None, sc=1., starting_location=(0, 0), size=(9, 9)):
    timer = utils.Timer()
    timer.tic()
    ...
    while len(to_traverse) != 0:
        n = to_traverse.popleft()
        for d in _get_next_nodes(n, sc, size):
            if valid_fn_vec(d) and not G.has_node(d):
                G.add_node(d)
                G.add_edge(n, d)
                to_traverse.append(d)
